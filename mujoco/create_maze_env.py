...
class GoalWrapper(Wrapper):
    ...
    def __init__(self, env, maze_size_scaling, random_start, goal, dense_reward_type, low, high):
        ...
        self.distance = 0
        self.distance_threshold = 4 * maze_size_scaling / 8.
        ...
        self.start_point = [0, 2 * maze_size_scaling]

    def set_randomstart(self, ifrandom):
        self.random_start = ifrandom

    def step(self, action):
        observation, reward, done, info = self.env.step(action)
        out = {'observation': observation, 'desired_goal': self.goal, 'achieved_goal': observation[..., :self.goal_dim]}
        distance = -np.linalg.norm(observation[..., :self.goal_dim] - self.goal, axis=-1)
        info['is_success'] = (distance > -self.distance_threshold)

        if self.dense_reward_type == 0:
            reward = self.compute_rew(observation[..., :self.goal_dim], self.goal, '...')
        elif self.dense_reward_type == 1:
            reward = distance - self.distance
        elif self.dense_reward_type == 2:
            reward = distance / (34 * self.maze_size_scaling / 8.)  # -distance / max_distance
        else:
            raise ("error dense reward type")
    ...
    def reset(self):
        ...
        if self.goal is None:
            ...
            if self.env._is_in_collision(self.goal):
                assert ('impossible goal')

        # random start a position without collision
        if self.random_start:
            xy = self.maze_space.sample()
            while (self.env._is_in_collision(xy) or max(abs(xy[0] - self.goal[0]), abs(xy[1] - self.goal[1])) < self.distance_threshold):
                xy = self.maze_space.sample()
            self.env.wrapped_env.set_xy(xy)
            observation = self.env._get_obs()
        else:
            self.env.wrapped_env.set_xy(self.start_point)
            observation = self.env._get_obs()

        out = {'observation': observation, 'desired_goal': self.goal}
        self.distance = np.linalg.norm(observation[..., :self.goal_dim] - self.goal, axis=-1)
...
def create_maze_env(env_name=None,
    ...
    return GoalWrapper(gym_env, maze_size_scaling, random_start, goal, dense_reward_type, *goal_args)
